...
OUTPUT_ROOT = REPO_ROOT / "outputs" / "runs" / "three_lane_demand_calibration"
...
def configure_rate(rate: int, episodes: int) -> Path:
    ...
    route_path = results_dir / f"grid_4x4_3lane_rate{rate}.rou.xml"
    set_constant("RATE", rate)
    set_constant("ROUTE_FILE_PREFIX", "grid_4x4_3lane")
    set_constant("ROUTE_FILE", str(route_path))
    set_constant("SCENARIO_DIR", "scenarios/grid_4x4_3lane")
    set_constant("SUMO_CONFIG", "sim.sumocfg")
    set_constant("NETWORK_FILE", "grid_4x4_3lane.net.xml")
    set_constant("ACTION_SPACE", "three_lane_8")
    ...
    set_constant("ADP_VARIANT_LABEL", "three_lane_8_action_lane_fairness_v1")
...
def main() -> None:
    scenario_dir = REPO_ROOT / "scenarios" / "grid_4x4_3lane"
    os.chdir(scenario_dir)
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    rates = list(config.THREE_LANE_RATE_CANDIDATES)
    episodes = 1
    all_rows: list[dict[str, str]] = []

    for rate in rates:
        print(f"=== THREE-LANE DEMAND CALIBRATION rate={rate} ===")
    ...
    print(f"Selected three-lane rate: {selected_rate}")
