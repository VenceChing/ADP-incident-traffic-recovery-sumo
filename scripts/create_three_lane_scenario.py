...
def _lane_for_turn(turn_dir: str) -> str:
...
def _set_connection_lanes(connection_path: Path, output_path: Path, turn_dirs: dict[tuple[str, str], str]) -> None:
    ...
    for connection in root.findall("connection"):
        ...
        lane = _lane_for_turn(turn_dir)
        connection.set("fromLane", lane)
        connection.set("toLane", lane)
    tree.write(output_path, encoding="utf-8", xml_declaration=True)


def create_three_lane_scenario(source_dir: Path, target_dir: Path) -> None:
    source_net = source_dir / "grid_4x4.net.xml"
    source_route = source_dir / "grid_4x4_rate2500.rou.xml"
    target_net = target_dir / "grid_4x4_3lane.net.xml"

    target_dir.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="three_lane_plain_") as tmp_name:
        ...
        three_lane_edges = tmp_dir / "grid_3lane.edg.xml"
        three_lane_connections = tmp_dir / "grid_3lane.con.xml"

        _set_edge_lanes(edge_path, three_lane_edges)
        _set_connection_lanes(connection_path, three_lane_connections, _connection_dirs(source_net))
        ...
        subprocess.run(
            [
                ...
                str(node_path),
                "-e",
                str(three_lane_edges),
                "-x",
                str(three_lane_connections),
                "-o",
                str(target_net),
                "--tls.guess",
                "true",
    ...
    if source_route.exists():
        shutil.copy2(source_route, target_dir / "grid_4x4_3lane_rate2500.rou.xml")
    ...
    (target_dir / "sim.sumocfg").write_text(
        "\n".join(
            [
                "<configuration>",
                "    <input>",
                '        <net-file value="grid_4x4_3lane.net.xml"/>',
                '        <route-files value="grid_4x4_3lane_rate2500.rou.xml"/>',
                "    </input>",
                "    <time>",
                '        <begin value="0"/>',
                "    </time>",
    ...
    issues = validate_three_lane_network(target_net)
    if issues:
        raise RuntimeError("Generated scenario failed validation:\n" + "\n".join(issues[:20]))


def main() -> None:
    parser = argparse.ArgumentParser(description="Create the three-lane grid_4x4 SUMO scenario.")
    parser.add_argument("--source-dir", default=str(REPO_ROOT / "scenarios" / "grid_4x4"))
    parser.add_argument("--target-dir", default=str(REPO_ROOT / "scenarios" / "grid_4x4_3lane"))
    args = parser.parse_args()
    create_three_lane_scenario(Path(args.source_dir), Path(args.target_dir))
