...
The GUI demos intentionally use denser demo-only traffic (`RATE: 5000`, `TIME: 1180`, incident at step `150`) and fixed high-flow incident edges so the vehicles and controller differences are visible on screen. This does not change the final reported evaluation runs, which remain under the `real_world*_final_reproduction_rate1000` output folders.
